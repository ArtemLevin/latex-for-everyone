...
- Добавить confirm перед заменой активного файла.
- Добавить режимы вставки результата:
  - заменить текущий файл;
  - создать новый файл `generated.tex`;
  - вставить в конец текущего файла.
- Показывать loading state на кнопках `Проверить prompt`, `Проверить провайдера`, `Проверить .tex`, `Сгенерировать и вставить`.
- Показать provider/model status badge в AI-модалке.
- Добавить кнопки `Скопировать prompt` и `Скопировать raw output`.
- Отображать validation errors/warnings списком, а не одной строкой.
