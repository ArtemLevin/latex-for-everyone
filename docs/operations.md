...
| Full readiness summary | `GET /api/ready` | `ready`, `degraded` or `not_ready` |
...
|---------|--------------|-------------|
| `429 Too Many Requests` from `/api/generation/*` | Per-client AI rate limit was exceeded. | Wait for `Retry-After`; do not add frontend auto-retry loops. If this happens during normal use, review duplicate submits and `AI_RATE_LIMIT_PER_MINUTE`. |
...
- In-memory AI rate limiting is process-local; use a shared store such as Redis
  before horizontally scaling API replicas.
